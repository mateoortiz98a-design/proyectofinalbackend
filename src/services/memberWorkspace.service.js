...
class MemberWorkspaceService {
    ...
    async inviteUser(user_invited_for_id, user_invited_email, workspace_id, role) {
        ...
        // Obtener info del workspace para la notificación
        const workspace = await workspaceRepository.getById(workspace_id)

        // Emitir notificación en tiempo real al usuario invitado
        const io = getIO()
        io.to(`user:${userToInvite._id}`).emit('workspace_invitation', {
            member_id: member_created._id,
            workspace_id,
