...
class PrivateMessageService {
    ...
    async getMessageById(message_id) {
        ...
        const message =
            await privateMessageRepository.getById(message_id);
    ...
    async createMessage(chat_id, sender_id, mensaje) {
        ...
        if (!isParticipant) {
            throw new Error("No perteneces a esta conversación.");
        }

        return await privateMessageRepository.create(
            chat_id,
            sender_id,
            mensaje.trim()
        );
    ...
    async updateMessage(message_id, sender_id, update_data) {
        ...
        if (
            message.fk_sender_user_id._id.toString() !==
            sender_id.toString()
        ) {
        ...
        return await privateMessageRepository.updateById(
            message_id,
            update_data
        );
    }
